...
TEST_RUN = False
PARENT_URL = 'https://www.cvmarket.lt/darbo-skelbimai-pagal-data?submit=1&start='
OUTPUT_FILENAME = "/jobad_scrape/data/" + timestamp_extension() + ".txt"
LOG_FILENAME = "/jobad_scrape/log/ads_log_cvmarketlt.txt"
...
#countsDict = get_counts_cvonlinelt(PARENT_URL + '0')
...
while True:
    ...
    # concatenate web url
    urlConstruct = PARENT_URL + str(pageUrl)

    # web query
    html = make_soup(urlConstruct)

    # break if no more content found
    adDataRows = adContentExist(html, 'tr', 'f_job_row')
    if not adDataRows:
        # print break message
    ...
    adData = scrape_cvmarketlt(adDataRows)
    # writes to text file
